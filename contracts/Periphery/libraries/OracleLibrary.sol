...
library OracleLibrary {
    /// @notice Calculates time-weighted means of tick and liquidity for a given SwifyDex  pool
        ...
        returns (int24 arithmeticMeanTick, uint128 harmonicMeanLiquidity)
    {
        ...
        ) = ISwifyDexPool(pool).observe(secondsAgos);
    ...
    /// @param pool Address of SwifyDex  pool that we want to observe
    ...
    function getOldestObservationSecondsAgo(
        ...
        ) = ISwifyDexPool(pool).slot0();
        require(observationCardinality > 0, "NI");

        (uint32 observationTimestamp, , , bool initialized) = ISwifyDexPool(
            pool
        ...
        if (!initialized) {
            (observationTimestamp, , , ) = ISwifyDexPool(pool).observations(0);
        }
    ...
    /// @param pool Address of SwifyDex  pool
    ...
    function getBlockStartingTickAndLiquidity(
        ...
        ) = ISwifyDexPool(pool).observations(observationIndex);
        if (observationTimestamp != uint32(block.timestamp)) {
            return (tick, ISwifyDexPool(pool).liquidity());
        }
        ...
        ) = ISwifyDexPool(pool).observations(prevIndex);
